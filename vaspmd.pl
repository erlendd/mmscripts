...
system ("zgrep POTCAR $outcar[0] > tmpfile2");
open (TMP2, tmpfile2);
$i=1;
while (<TMP2>){
@field=split (' ');
...
for ($i=1; $i<=$nat; $i++){
system ("zgrep 'ions per type' $outcar[0] > tmpfile2");
open (TMP2, tmpfile2);
while (<TMP2>){
@field=split(' ');
$atn[$i]=$field[$i+3];
...
system ("zgrep -A $matches POSITION $outcar[$i] | grep -v '\\-\\-' | grep -v POSITION >> tmpfile2");
};
open (TMP2, tmpfile2);
$p=1;
while (<TMP2>){
...
# print [time] [E] [F] [Ek] [T]:
printf OUT "%3.4f  $e[$i]  $f[$i]  $ek[$i]  $t[$i]\n", $time;
}; 
close (OUT);
...
print OUT2 "plot \"vaspmd.E.out\" u 1:2 t \"Total free energy (F + Ek)\" with lines, \\\n";
print OUT2 "\"vaspmd.E.out\" u 1:3 t \"Free Energy (F)\" with lines \n";
close (OUT2);
